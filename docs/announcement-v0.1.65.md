# Announcement drafts — v0.1.65

## Short

v0.1.65 certifies all 28 packages against Pi 0.84.2 and refreshes the oracle and contrarian provider catalogs with `baseten` (top pick: `moonshotai/Kimi-K3`) and `qwen-token-plan-individual` (top pick: `qwen3.8-max`). Top picks are also upgraded for three providers: `xai` now picks `grok-4.6` (up from `grok-4.5`) and `zai`/`zai-coding-cn` now pick `glm-5.3` (up from `glm-5.2`). Two silent regressions are fixed: Pi renamed `qwen3.8-max-preview` to `qwen3.8-max`, causing preference matching to fall through to `qwen3.7-max` for `qwen-token-plan` and `qwen-token-plan-cn` users — both are now back on `qwen3.8-max`. Fast-mode allowlists are also retargeted: `/fast` gains `claude-opus-5` and goes inert on `claude-opus-4-6`, `claude-opus-4-7`, and `gpt-5.4`.
