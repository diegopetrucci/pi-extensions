# Publish checklist — v0.1.16

## Preflight

- [x] confirm working tree contains only intended changes
- [x] verify versions are correct
  - [x] `package.json` -> `0.1.16`
  - [x] `extensions/compact-bash/package.json` -> `0.1.0`
...
- [x] verify docs exist
  - [x] `docs/release-notes-v0.1.16.md`
  - [x] `docs/github-release-v0.1.16.md`
  - [x] `docs/announcement-v0.1.16.md`
  - [x] `docs/publish-checklist-v0.1.16.md`
- [x] verify extension files exist
  - [x] `extensions/compact-bash/index.ts`
...
bun --check extensions/compact-bash/index.ts
...
```bash
PI_OFFLINE=1 pi --no-extensions --extension ./extensions/compact-bash/index.ts --list-models smoke
...
- [x] run compact-bash renderer smoke test

```bash
export NODE_PATH=/opt/homebrew/lib/node_modules/@earendil-works/pi-coding-agent/node_modules:/opt/homebrew/lib/node_modules
bun --eval '<loads compact-bash, verifies one collapsed line, expanded output, and narrow-width rendering>'
```
...
- [x] confirm collection package exposes all seven extensions in root `package.json`
- [x] confirm standalone package metadata uses `@earendil-works/*` peer dependencies
- [x] confirm compact-bash package declares `@earendil-works/pi-coding-agent` and `@earendil-works/pi-tui` peers
...
```bash
git commit -m "Prepare v0.1.16 release"
git tag v0.1.16
```
...
- [ ] create GitHub release for `v0.1.16`
- [ ] paste contents of `docs/github-release-v0.1.16.md`
...
- [ ] publish standalone compact-bash package

```bash
cd extensions/compact-bash
npm publish
cd ../..
```
...
- [ ] verify npm package pages show expected versions
  - [ ] `@diegopetrucci/pi-extensions@0.1.16`
  - [ ] `@diegopetrucci/pi-compact-bash@0.1.0`
...
- [ ] publish announcement copy from `docs/announcement-v0.1.16.md`
