Packages the Pi `0.82.0` compatibility update for the root collection and 26 standalone tarball-changed extensions.

## Highlights

- removes the unavailable `mistralai/mistral-large-3-675b-instruct-2512` provider preference from `@diegopetrucci/pi-contrarian` and `@diegopetrucci/pi-oracle`
- updates all 27 selected `.pi-fleet-tested-version` markers to Pi `0.82.0`
- bumps the selected packages to their next unpublished patch versions through the deterministic release-prep flow
- final tlh validation passed with 505/505 full-suite tests, 4/4 focused provider-catalog checks, 1/1 tarball/runtime smoke coverage, unpublished-version confirmation for all 27 targets, and clean pack/publish dry-runs for the root package plus all 26 standalone targets
