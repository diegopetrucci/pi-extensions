Packages the Pi `0.82.1` compatibility update for the root collection and 26 standalone tarball-changed extensions.

## Highlights

- prefers `claude-opus-5` ahead of older Claude frontier fallbacks in `@diegopetrucci/pi-oracle`, `@diegopetrucci/pi-contrarian`, and `@diegopetrucci/pi-code-reviewer`
- updates the supporting provider policy contract coverage, oracle provider matrix, and agent workflow audit renderer compatibility for Pi `0.82.1`
- updates all 27 selected `.pi-fleet-tested-version` markers to Pi `0.82.1`
- bumps the selected packages to their next unpublished patch versions through the deterministic release-prep flow
- final tlh validation passed with 519/519 full-suite tests, 4/4 focused provider-catalog checks, 1/1 tarball/runtime smoke coverage, unpublished-version confirmation for all 27 targets, and clean pack/publish dry-runs for the root package plus all 26 standalone targets
