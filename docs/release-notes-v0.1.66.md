...
- `permission-gate` now guards Pi's optional `powershell` tool. It combines bounded conservative lexical checks on every platform with native PowerShell AST analysis on Windows, fails closed on malformed or dynamic command forms, and preserves benign quoted/commented text, literal here-strings, and definitely enabled `-WhatIf` removals.
...
- Deterministic release preparation selected all 28 unpublished next-patch targets and preserved all four managed release documents on its final rerun. Package dry-runs total 319 files, 2,483,235 packed bytes, and 4,281,205 unpacked bytes; the 163-file root is 1,288,387 packed / 2,214,459 unpacked bytes and remains within its 2,225,000-byte unpacked budget.
