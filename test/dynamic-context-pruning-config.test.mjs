...
test('supersededFileOps tool-name lists default to the pi built-ins and are config-overridable', async (t) => {
  setupAgentDir(t);
  const dcp = await import(pathToFileURL(modulePath).href);

  const defaults = dcp.defaultConfig().strategies.supersededFileOps;
  assert.deepEqual(defaults.readToolNames, ['read']);
  assert.deepEqual(defaults.writeToolNames, ['write', 'edit']);
